...
$('#addIncomeBtn').click(function () {
    ...
    if(maxFieldNumber == 1) {
            ...
            $.ajax({
                method : "GET",
                url : 'api/select-owners-equity-account',
                dataType : 'JSON',
                success : function (data) {
                    for (var i=0; i<=Object.keys(data).length-1; i++) {
...
function createNewAddIncome() {
    ...
    $.ajax({
        method : "POST",
        url : 'api/new-income-transaction',
        dataType : 'JSON',
        data : {
        ...
        success : function (data) {
            alert(data);
            window.location = "/account-app-ela/public/add-transaction";
        }
    });
}
...
$('#addExpenseBtn').click(function () {
        ...
        if(expenseFieldNumber==1) {
            ...
            $('#newWrapperForTransaction').append(
                ...
                '</div>' +
                '<div class="col-md col-sm-12">' +
                '<select name="payment_type" id="expenseTransactionPaymentType" class="form-control">' +
                '<option> --- Select Payment Method --- </option>' +
                '<option value="Bank Payment">Bank Payment</option>' +
                '<option value="Cash">Cash</option>' +
                '<option value="Check">Check</option>' +
            ...
            $.ajax({
                method : "GET",
                url : 'api/select-expense-account',
                dataType : 'JSON',
                success : function (data) {
                    // incomeAccounts = data;
                    for (var i=0; i<=Object.keys(data).length-1; i++) {
    ...
    function createNewAddExpense() {
        ...
        $.ajax({
            method : "POST",
            url : 'api/new-expense-transaction',
            dataType : 'JSON',
            data : {
                _token: $('input[name="_token"]').val(),
    ...
    function selectLiabilitiesAccount(x) {
        $.ajax({
            method : "GET",
            url : 'api/select-liabilities-account',
            dataType : 'JSON',
            success : function (data) {
                for (var i=0; i<=Object.keys(data).length-1; i++) {
    ...
    function selectOwnersEquityAccount(x) {
        $.ajax({
            method : "GET",
            url : 'api/select-journal-owners-equity-account',
            dataType : 'JSON',
            success : function (data) {
                for (var i=0; i<=Object.keys(data).length-1; i++) {
    ...
    $('#addALine').click(function () {
        ...
        $('#jt-'+indexNo).after(
            ...
            '<optgroup label="Asset Account" id="journalAssetAccount'+indexNo+'">' +
            '</optgroup>' +
            '<optgroup label="Liabilities Account" id="journalLiabilitiesAccount'+indexNo+'">' +
            '</optgroup>' +
            '<optgroup label="Income Account" id="journalIncomeAccount'+indexNo+'">' +
            '</optgroup>' +
            '<optgroup label="Expense Account" id="journalExpenseAccount'+indexNo+'">' +
            '</optgroup>' +
            '<optgroup label="Owners Equity Account" id="journalOwnersEquityAccount'+indexNo+'">' +
            '</optgroup>' +
            '</select>' +
            '</td>' +
            '<td class="p-l-20 p-r-20 p-t-10 p-b-10"><textarea class="form-control" name="transaction['+indexNo+'][account_description]"></textarea></td>' +
...
$('.edit-journal').click(function () {
    ...
    $('#jt-'+editJournalIndexNo).after(
        ...
        '</select>' +
        '</td>' +
        '<td class="p-l-20 p-r-20 p-t-10 p-b-10"><textarea class="form-control" name="transaction['+editJournalIndex+'][account_description]"></textarea></td>' +
...
$('.verified-transaction').click(function () {
    ...
    $.ajax({
        method: "GET",
        url: "api/verified-transaction/"+transactionId,
        dataType: 'JSON',
        success: function (data) {
            alert(data);
            window.location = "/account-app-ela/public/add-transaction";
...
$('.not-verified-transaction').click(function () {
    ...
    $.ajax({
        method: "GET",
        url: "api/not-verified-transaction/"+transactionId,
        dataType: 'JSON',
        success: function (data) {
            alert(data);
            window.location = "/account-app-ela/public/add-transaction";
...
$('#searchTransactionStatus').change(function () {
    ...
    if(searchItem == ' '){
        ...
        $.ajax({
            method : "GET",
            url : 'api/get-transaction-by-search-item/'+searchItem,
            dataType : 'JSON',
            success : function (data) {
...
$('#searchTransactionType').change(function () {
    ...
    if(searchItem == ' '){
        ...
        $.ajax({
            method : "GET",
            url : 'api/get-transaction-by-search-type/'+searchItem,
            dataType : 'JSON',
            success : function (data) {
                $('#outputResult').html(data);
            }
...
$('#searchTransactionAccountCategory').change(function () {
    ...
    if(searchItem == ' ') {
        ...
        $.ajax({
            method : "GET",
            url : 'api/get-transaction-by-account-category/'+searchItem,
            dataType : 'JSON',
            success : function (data) {
                $('#outputResult').html(data);
            }
...
$('#searchTransactionPaymentAccount').change(function () {
    ...
    if(searchItem == ' ') {
        ...
        $.ajax({
            method : "GET",
            url : 'api/get-transaction-by-payment-account/'+searchItem,
            dataType : 'JSON',
            success : function (data) {
                $('#outputResult').html(data);
            }
...
$('#find').click(function () {
    ...
    if(startDate == '' || endDate== '') {
        ...
        $.ajax({
            method : "GET",
            url : 'api/get-transaction-by-date/'+startDate+'/'+endDate,
            dataType : 'JSON',
            success : function (data) {
                $('#outputResult').html(data);
            }
...
$('#singleTransactionBtn').click(function () {
    ...
    if(transactionAccountCode == ' ' || transactionsStartDate == '' || transactionsEndDate == '') {
        ...
        $.ajax({
            method : "GET",
            url : 'api/get-transaction-by-account-code/'+transactionAccountCode+'/'+transactionsStartDate+'/'+transactionsEndDate,
            dataType : 'JSON',
            success : function (response) {
                if(response != '') {
                    ...
                    $.ajax({
                        method : "GET",
                        url : 'api/get-account-name-by-code/'+transactionAccountCode,
                        dataType : 'JSON',
                        success : function (response) {
